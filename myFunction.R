myFunction = function (moveInfo, readings, positions, edges, probs){
  ...
  # checks if the second hicker is dead
    ...
    #normalizing
    probability[positions[1]] = 0
    probability[positions[2]] = 0
  ...
  if(  moveInfo$moves[1] == 0){
    ...
    moveInfo$mem$probNodes = moveInfo$mem$probNodes/sum(probability)
    # normalize probability
    if(holeVar != 0 ){
    moveInfo$moves[2] = moveMatrix[positions[3],index[1],1]
    }else{
      moveInfo$moves[2]=moveMatrix[positions[3],index[2],1]
    }
  }
